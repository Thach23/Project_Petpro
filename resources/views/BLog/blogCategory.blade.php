    ...
    <main>
        ...
        <div class="container mx-auto">
            <div class="py-10 text-sm md:text-lg">
                ...
                <h1 class="text-blue-200 text-4xl md:text-[42px] capitalize leading-tight">Kiến thức</h1>
            </div>
        </div>
        </div>
                ...
                @foreach ($BlogCategories as $item)
                    @php
                        ...
                        foreach ($DsBlog as $blog) {
                            if ($item->Id == $blog->BlogCategoryId) {
                                $count++;
                            }
                        }
                    @endphp
                    ...
                    <div
                        ...
                        <a href="{{ route('KienThuc', ['slug' => $item->Slug]) }}">
                            {{ $item->Name }}
                        </a>
                    </div>


                    @if ($DsBlog == null || $count == 0)
                        <p>Không có dữ liệu</p>
                    @else
                        <div class="grid gap-6 lg:grid-cols-3">
                            @foreach ($DsBlog as $val)
                                @if ($val->BlogCategoryId == $item->Id)
                                    <div class="border border-gray-180 rounded-[5px] h-full flex flex-col">
                                        <a href="{{ route('blogdetail', ['slug' => $val->Slug]) }}">
                                            @if ($val->BlogImage == null)
                                                <img class="w-full rounded-t-[5px] flex-shrink-0"
